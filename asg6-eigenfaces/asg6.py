...
def distanceImages(dataImg1, dataImg2):
    ...
    distanceArray = dataImg1 - dataImg2
    distance = math.sqrt(np.sum(distanceArray**2))
    return distance
...
def projectAndBackProject(dataImg, mean, eigenVectors):
    ...
    """
    handle = False
    if (dataImg.shape == mean[0].shape):
        dataImg = np.array([dataImg])
        handle = True
    projected = cv2.PCAProject(dataImg, mean, eigenVectors)
    back = cv2.PCABackProject(projected, mean, eigenVectors)
    if (handle):
        return back[0]
    return back
...
def indexDistancePairsSortedByDistance(data, r, mean, eigenVectors):
    ...
    """
    PAB = projectAndBackProject(data, mean, eigenVectors)
    out = [(0,0.0)]*len(r)

    for i in r:
        distance = distanceImages(data[i], PAB[i])
        temp = (i, distance)
        out[i] = temp
