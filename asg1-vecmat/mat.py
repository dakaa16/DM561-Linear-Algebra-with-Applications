...
def add(A, B):
    ...
    """
    assert A.size == B.size
    a = [[0 for x in range(A.size[1])] for x in range(A.size[0])]
    for i in range(A.size[0]):
        for j in range(A.size[1]):
            a[i][j] = getitem(A, [i,j]) + getitem(B, [i,j])
    c = Mat(a);
    return c
...
def transpose(M):
    ...
    True
    """
    c = Mat([[0 for x in range(M.size[0])] for x in range(M.size[1])])
    for i in range(M.size[0]):
        for j in range(M.size[1]):
            setitem(c, [j,i], getitem(M, [i,j]))
    return c
...
def vector_matrix_mul(v, M):
    ...
    c = Vec([0 for x in range(M.size[1])])
...
def matrix_vector_mul(M, v):
    ...
    c = Vec([0 for x in range(M.size[0])])
    for i in range(M.size[0]):
        a=Vec([0]*v.size)
        for j in range(v.size):
            a[j] = getitem(M, [i, j])
...
def matrix_matrix_mul(A, B):
    ...
    c = Mat([[0 for x in range(B.size[1])] for x in range(A.size[0])])
    for i in range(A.size[0]):
        a = Vec([0]*A.size[1])
        for j in range(A.size[1]):
            a[j] = getitem(A, [i, j])
