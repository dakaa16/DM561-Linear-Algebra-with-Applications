...
def initPoints(num):
    ...
    if num <= 2:
        raise ValueError("num has to be bigger than 2")

    x = np.random.random_sample(num)
    y = np.random.random_sample(num)

    x = np.subtract(x, mean(x))
    y = np.subtract(y, mean(y))

    x = np.divide(x, norm(x))
    y = np.divide(y, norm(y))
...
def invMatrixM(M):
    ...
    """
    if M.shape[0] % 2 == 0:
        raise ValueError("The number of rows or columns should be odd")
...
def updatePoints(M, x, y):
    ...
    """
    if np.around(norm(x),6) != 1.0 or np.around(norm(y),6) != 1.0:
        raise ValueError("L2 norm of x or y is not 1.0")

    vx = np.dot(M, x)
    vy = np.dot(M, y)

    newx = np.divide(vx, norm(vx))
    newy = np.divide(vy, norm(vy))
